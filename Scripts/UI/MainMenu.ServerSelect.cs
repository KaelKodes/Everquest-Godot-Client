...
public partial class MainMenu : Control
{
    ...
    //   Mithaniel Marr → dev cluster on the same WAN, different forwarded port
    //                    (4005 vs 3005).
    //
    ...
    // Returns the server list either from the built-in defaults or from
    // user://servers.cfg if present. Each entry may have multiple candidate URLs.
    //
    // servers.cfg format (UTF-8, JSON array). Either "url" (single) or
    // "urls" (array) is accepted per entry; "urls" wins if both are present.
    //   [
    //     {"name":"Mithaniel Marr",
    //      "urls":["ws://127.0.0.1:4005","ws://192.168.1.50:4005","ws://24.33.88.252:4005"]},
    //     {"name":"Erollisi Marr","url":"ws://24.33.88.252:3005"}
    //   ]
    private static List<(string DisplayName, List<string> Urls)> LoadServerListOverrides()
    {
        List<(string, List<string>)> DefaultList()
        {
        ...
        try
        {
            ...
            {
                GD.PrintErr("[SERVERSELECT] servers.cfg must be a JSON array; using defaults.");
                return DefaultList();
            }

            var fromFile = new List<(string, List<string>)>();
            foreach (var el in doc.RootElement.EnumerateArray())
            {
                ...
                fromFile.Add((name.Trim(), urls));
            }

            if (fromFile.Count == 0)
            {
                GD.PrintErr("[SERVERSELECT] servers.cfg had no valid entries; using defaults.");
                return DefaultList();
            }

            GD.Print($"[SERVERSELECT] Loaded {fromFile.Count} server(s) from servers.cfg");
            return fromFile;
        }
        catch (Exception ex)
        {
            GD.PrintErr($"[SERVERSELECT] servers.cfg read failed ({ex.Message}); using defaults.");
            return DefaultList();
        }
    }
